...
#read data
united_traits = read.delim('c:/ron/tmp/united_09.txt', header = TRUE)
traits_with_age = read.csv('c:/ron/tmp/traits_with_age_for_R_09.csv')
#add -1 to NULLS
...
for (i in c(1:n)){
  ...
  mixed_model = lme(trait1~Age,data=subset_for_mixed, random=~1|trait2)
...
write.csv(x = ret_df,file = 'c:/ron/tmp/r_output_tresh_09.csv')
