...
#read data
united_traits = read.delim('C:/Ron/tmp/R/tresh_06/united_06_part1.txt', header = TRUE)
traits_with_age = read.csv('C:/Ron/tmp/R/tresh_06/traits_with_age_for_R_tresh_06.csv')
#add -1 to NULLS
...
for (i in c(1:n)){
  ...
  mixed_model = tryCatch({lme(trait1~Age,data=subset_for_mixed, random=~1|trait2, control=lmeControl(returnObject=TRUE))},
...
write.csv(x = ret_df,file = 'C:/Ron/tmp/R/tresh_06/R_output_tresh_06_part1_run1.csv')
